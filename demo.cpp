...
void draw_pixel(int x, int y, float color[], float width) {
	glPointSize(width);
	glBegin(GL_POINTS);
...
void DDAalgorithm(){
	...
	//first point
	double x =1.0 * xStart,y=1.0 * yStart;//Convert to double
...
void Bresenham() {
	int dx, dy, i, e,incx, incy, inc1, inc2, x,y;  float color[3],point_size=10.0;
	...
	x = xStart; y = yStart;
	if (dx > dy) {
		draw_pixel(x, y,color,point_size); e = 2 * dy-dx; inc1 = 2*(dy-dx); inc2 = 2*dy;
		for (i=0; i<dx; i++) {
			if (e >= 0) {
				y += incy; e += inc1; }
			else e += inc2;
			x += incx; draw_pixel(x, y,color,point_size); }
	} else {
		draw_pixel(x, y,color,point_size); e = 2*dx-dy; inc1 = 2*(dx-dy); inc2 = 2*dx;
		for (i=0; i<dy; i++) {
			if (e >= 0) {
				x += incx; e += inc1; }
			else e += inc2;
			y += incy; draw_pixel(x, y,color,point_size); } }
}
